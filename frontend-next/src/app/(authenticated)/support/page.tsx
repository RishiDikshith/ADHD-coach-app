...
const fallbackFaqs = [
  {
    ...
    answer:
      "When task paralysis strikes, your brain is treating the threat of starting a task like a literal physical danger. Don't fight it! Give yourself absolute permission to do the task badly or do just one single detail for 2 minutes. Start a micro-timer, and if you want to stop after 2 minutes, you have fully succeeded.",
  },
  {
    id: "faq-hyperfocus",
    question: "🌀 Help, I am stuck in an intense hyperfocus spiral!",
    answer:
      "Hyperfocus is a powerful ADHD gift, but it can drain your body. Transitioning out is hard. Use a transitional bridge: instead of stopping immediately, tell yourself you will stop in 5 minutes, stand up and stretch without looking away, then grab a glass of water. A change of physical state helps reset the brain.",
  },
  {
    id: "faq-blindness",
    question: "⏳ How can I handle time blindness during work?",
    answer:
      "ADHD brains perceive time as 'Now' or 'Not Now'. To make time visible, use visual timers (like the standard countdown visual arc in our Focus page) rather than digital numbers. Set soft, chime-based alarms 5 minutes before you actually need to transition to prevent jump-scares.",
  },
  {
    id: "faq-burnout",
    question: "🔋 What is an ADHD shutdown, and how do I recover?",
    answer:
      "When you have overstimulated or pushed your brain too hard, it goes into a power-saving mode (shutdown/burnout). This is a physical necessity. Rest shame-free. Lie down in a dark or quiet room, drink some hydration, and avoid complex decision-making for at least 1-2 hours.",
  },
];
...
export default function SupportPage() {
  ...
  >([
    {
      role: "assistant",
      content:
        "Welcome to Support! I am your AI Support Companion. Whether you're experiencing a glitch, have a feature suggestion, or need shame-free answers to ADHD FAQs, I've got your back. How can I assist you today? 🆘",
    },
  ]);
    ...
    try {
      ...
      if (response.success) {
        ...
        setChatMessages((prev) => [
          ...
          {
            role: "assistant",
            content: `Thank you! I see that ticket #${response.ticket.id} has been successfully logged with status **OPEN**. I've cataloged this in our technical log, and our systems are ready to inspect it! 🚀`,
          },
        ]);
        loadTickets();
      }
      ...
      setTicketSuccess(
        "Offline mode active: Ticket saved locally. No pressure, our technical coach has noted this! 💖"
      );
      setSubject("");
      setDescription("");
    } finally {
    ...
    try {
      ...
      if (res.reply) {
        ...
        setChatMessages((prev) => [
          ...prev,
          {
            role: "assistant",
            content:
              "I received your message, but had trouble compiling a direct response. Our engineering team has been notified!",
          },
        ]);
      }
    } catch (err) {
      ...
      setChatMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          content:
            "Apologies, I encountered a temporary connection issue. Don't worry, your data is safe and no tickets have been lost!",
        },
      ]);
    } finally {
      setIsThinking(false);
  ...
  const getStatusBadge = (status: string) => {
    switch (status.toLowerCase()) {
      case "resolved":
        return (
          <Badge className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 font-bold uppercase text-[9px] tracking-wider">
            Resolved 🟢
          </Badge>
        );
      case "in_progress":
        return (
          <Badge className="bg-blue-500/10 text-blue-400 border border-blue-500/30 font-bold uppercase text-[9px] tracking-wider">
            In Progress 🔵
          </Badge>
        );
      default:
        return (
          <Badge className="bg-amber-500/10 text-amber-400 border border-amber-500/30 font-bold uppercase text-[9px] tracking-wider">
            Open 🟡
          </Badge>
        );
    }
      ...
      >
        <div>
          ...
          <p className="text-muted text-sm mt-1">
            Submit help desk glitch tickets, chat with AI Support, or explore calming ADHD coping
            strategies
          </p>
        </div>
      </motion.div>
        ...
        <motion.div variants={itemVariants} className="lg:col-span-4 space-y-6">
          ...
          <Card className="glass-strong border-pink-500/10 relative overflow-hidden">
            ...
            <CardDescription className="mt-1 text-xs">
              Encountered a bug or have a question? Submit a ticket and we will inspect the database
              logs.
            </CardDescription>
                ...
                faqs.map((faq) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <span className="font-semibold text-foreground hover:text-pink-400">
                          {faq.question}
                        </span>
        ...
        <motion.div variants={itemVariants} className="lg:col-span-5 h-[620px] flex flex-col">
          <Card className="glass-strong border-pink-500/20 bg-gradient-to-b from-[#0e0f1d] to-[#060711] flex flex-col h-full overflow-hidden p-0 relative">
            ...
            <div className="p-4 border-b border-pink-500/10 flex items-center gap-3 bg-[#0a0c16]/80 backdrop-blur-md shrink-0">
              <span className="text-3xl p-1.5 rounded-xl bg-pink-500/10 border border-pink-500/20 text-pink-400">
                🆘
              </span>
              <div>
            ...
            <form
              ...
              className="p-3 border-t border-pink-500/10 bg-[#0a0c16]/90 backdrop-blur-md flex items-center gap-2 shrink-0"
            >
              <input
                type="text"
                value={chatInput}
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="flex justify-between items-center text-[10px] text-muted-foreground w-full">
                          <span className="font-mono text-[9px] text-pink-400/80">
                      ...
                      <AnimatePresence>
                        {isExpanded && (
                          ...
                          >
                            <p className="font-medium text-foreground text-[10px] uppercase tracking-wider">
                              Description:
                            </p>
                            ...
                            {ticket.admin_notes && (
                              <div className="border-t border-dashed border-border/40 pt-2 space-y-1">
                                <p className="font-medium text-pink-400 text-[10px] uppercase tracking-wider">
                                  Admin Reply:
                                </p>
