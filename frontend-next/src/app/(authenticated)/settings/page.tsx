...
export default function SettingsPage() {
  ...
  const handleRemoveDevice = async (deviceIdToRemove: string) => {
    try {
      const res = await api.revokeTrustedDevice(deviceIdToRemove);
      if (res.success) {
        setPinMessage("🗑️ Device revoked successfully!");
        fetchDevices();
  ...
  const handleRevokeAllDevices = async () => {
    try {
      ...
      if (res.success) {
        setPinMessage(`🗑️ All devices revoked (${res.revoked_count} total).`);
        fetchDevices();
        setTimeout(() => setPinMessage(""), 3000);
      } else {
            ...
            <div className="mt-4 space-y-4">
              ...
              <div className="pt-4 mt-4 border-t border-border/40 w-full space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  {devices.length > 1 && (
                    <button
                      ...
                      className="text-[11px] font-medium text-danger-500 hover:text-danger-400 transition-colors cursor-pointer"
                    >
                      Revoke All Devices
                    </button>
                  )}
                ...
                {loadingDevices ? (
                  ...
                  <p className="text-xs text-muted">No trusted devices registered yet.</p>
                ) : (
      ...
      <motion.div variants={itemVariants}>
        <Card>
          ...
          <p className="text-xs text-muted mt-1 mb-4">
            Manage your single sign-on identity with Google.
          </p>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-muted">
                          {googleAcc?.connected ? googleAcc.email || "Connected" : "Not connected"}
                        </p>
                      </div>
                    </div>
                    <div>
                      {googleAcc?.connected ? (
                        <span className="px-2.5 py-1 rounded-full text-[10px] font-semibold bg-calm-500/10 text-calm-400 border border-calm-500/30">
                          Connected
                        </span>
                      ) : (
                        <span className="px-2.5 py-1 rounded-full text-[10px] text-muted border border-border/60">
                          Available
                        </span>
                      )}
                    </div>
