...
const agentsList = [
  {
    ...
    description:
      "Helps you break down your day, set priorities, and find natural momentum without the shame.",
  ...
  },
  {
    ...
    description:
      "Transforms huge, intimidating projects into tiny, bite-sized 2-minute steps to beat paralysis.",
    color: "#6366f1", // Indigo
  ...
  },
  {
    ...
    description:
      "Guides you through deep-work blocks, monitors distractions, and helps you recover focus.",
    color: "#f59e0b", // Amber
  ...
  },
  {
    ...
    description:
      "Provides a safe, shame-free space when you are exhausted, stressed, or feeling guilty for resting.",
    color: "#ec4899", // Rose
  ...
  },
  {
    ...
    description:
      "Your friendly, zero-judgment accountability partner to keep you moving consistently.",
    color: "#a855f7", // Purple
  ...
  },
  {
    ...
    description:
      "Helps you check in with your emotions, reflect on stress levels, and understand your mood patterns.",
    color: "#0ea5e9", // Sky
  ...
  },
  {
    ...
    description:
      "Designs low-friction routines and rewards consistency with ADHD-friendly dopamine loops.",
  ...
  },
  {
    ...
    description:
      "Helps you schedule study sessions, break down heavy subjects, and prepare for exams calmly.",
    color: "#06b6d4", // Cyan
...
interface ISpeechRecognition {
  ...
  onstart: (() => void) | null;
  onresult: ((event: SpeechRecognitionEvent) => void | Promise<void>) | null;
...
const WAVE_DURATIONS = [0.65, 0.95, 1.1, 0.75, 1.25, 0.85, 1.05, 0.7, 0.9];

function getVoiceLanguageFromSettings(lang?: string): string {
  switch (lang) {
    case "es":
      return "es-ES";
    case "fr":
...
export default function AgentsPage() {
  ...
  const voiceLanguage = selectedVoiceLanguage ?? getVoiceLanguageFromSettings(settings?.language);
  ...
  const voiceRecognitionRef = useRef<ISpeechRecognition | null>(null);
  ...
  const startVoiceSession = (agentId: string) => {
    ...
    if (settings.language === "es") greeting = `¡Hola! Soy tu ${agentName}. ¡Hablemos por voz!`;
      ...
      greeting = `Bonjour! Je suis ton ${agentName}. Parlons par voix!`;
    else if (settings.language === "de")
      greeting = `Hallo! Ich bin dein ${agentName}. Lass uns per Sprache chatten!`;
    else if (settings.language === "ja")
      greeting = `こんにちは！私はあなたの${agentName}です。音声で話しましょう！`;
  ...
  const getRecommendedAgent = () => {
    ...
    if (
      latestMood &&
      (latestMood.label === "Anxious" ||
        latestMood.label === "Frustrated" ||
        latestMood.label === "Worried")
    ) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted text-sm max-w-2xl mt-1 font-medium">
              Select one of our 8 ADHD-specialized companions to start chatting verbally or via
              standard messaging. Enjoy continuous hands-free voice loops.
            </p>
          </div>
        </div>
      </motion.div>
          ...
          return (
            ...
            >
              ...
              >
                ...
                <div
                  className={`absolute -right-16 -bottom-16 w-36 h-36 rounded-full bg-gradient-to-br ${agent.gradient} blur-2xl opacity-40`}
                />
      ...
      <motion.div variants={itemVariants}>
        <Card variant="glass">
          ...
          <p className="text-xs text-muted mt-1">
            Each skill level improves as you chat with specialized agents, build routines, and
            conquer task paralysis.
          </p>
          <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              ...
              <div
                key={tree.skill}
                className="p-4 rounded-xl bg-surface/50 border border-border/40 shadow-sm relative overflow-hidden group"
              >
                <div className="flex items-center gap-3 mb-2.5">
                  <span className="text-2xl p-1.5 rounded-lg bg-surface/75 border border-border/50 group-hover:scale-110 transition-transform">
                    {tree.emoji}
                  </span>
                  <div>
        ...
        {voiceSessionActive && activeVoiceAgent && (
          ...
          >
            ...
            >
              ...
              <div
                className={`absolute -right-32 -top-32 w-64 h-64 rounded-full bg-gradient-to-br ${activeVoiceAgent.gradient} blur-3xl opacity-30 pointer-events-none`}
              />
              <div
                className={`absolute -left-32 -bottom-32 w-64 h-64 rounded-full bg-gradient-to-br ${activeVoiceAgent.gradient} blur-3xl opacity-30 pointer-events-none`}
              />

              {/* Header */}
              <div className="flex items-center justify-between border-b border-border/40 pb-4 relative z-10">
                <div className="flex items-center gap-3">
                  <span className="text-3xl p-1.5 rounded-xl bg-surface border border-border/60 shadow-sm">
                    {activeVoiceAgent.emoji}
                  </span>
                  <div>
                    <h2 className="text-lg font-black text-foreground">{activeVoiceAgent.name}</h2>
                    <p className="text-[10px] text-muted font-semibold uppercase tracking-widest leading-none">
                      {activeVoiceAgent.specialty}
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="flex flex-col items-center justify-center py-6 space-y-5 relative z-10">
                ...
                <div className="flex items-center justify-center gap-1.5 h-10 w-48 overflow-hidden px-4">
                  {[...Array(9)].map((_, i) => {
                    const duration = WAVE_DURATIONS[i % WAVE_DURATIONS.length];
                    const heightRange = voiceIsRecording
                      ? [4, 32, 4]
                      : voiceIsSpeaking
                        ? [4, 20, 4]
                        : [4, 8, 4];
                    const bgColor = voiceIsRecording
