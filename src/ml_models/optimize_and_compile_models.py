...
try:
    ...
    tfidf = TfidfVectorizer(
        max_features=8000, ngram_range=(1, 2), min_df=2, max_df=0.95, sublinear_tf=True
    )
    ...
    nlp_pipeline = Pipeline([("vectorizer", tfidf), ("classifier", lr_mh)])
    ...
    print(
        f"[SUCCESS] Saved production pipeline: {prod_nlp_path} | Size: {os.path.getsize(prod_nlp_path)/1024:.2f} KB"
    )
...
try:
    ...
    print(
        f"[SUCCESS] Saved production model: {prod_cb_path} | Size: {os.path.getsize(prod_cb_path)/1024:.2f} KB"
    )
...
try:
    ...
    print(
        f"[SUCCESS] Saved production model: {prod_stud_path} | Size: {os.path.getsize(prod_stud_path)/1024:.2f} KB"
    )
...
try:
    ...
    text_col = (
        "cleaned_text"
        if "cleaned_text" in df_adhd.columns
        else "if_yes_please_list_these_difficulties_and_or_symptoms"
    )
    ...
    print(
        f"[SUCCESS] Saved production pipeline: {prod_adhd_path} | Size: {os.path.getsize(prod_adhd_path)/1024:.2f} KB"
    )
