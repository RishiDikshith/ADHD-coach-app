...
class TestMultiChatbotEcosystem(unittest.TestCase):
    ...
    def test_chatbot_registry_completeness(self):
        ...
        for agent_id in required_agents:
            self.assertIn(
                agent_id, AGENT_CONFIGS, f"Agent '{agent_id}' is missing from chatbot registry!"
            )
            ...
            self.assertTrue(
                len(config["description"]) > 0, f"Description for '{agent_id}' is empty"
            )
            ...
            self.assertTrue(
                len(config["text_gradient"]) > 0, f"Text Gradient for '{agent_id}' is empty"
            )
            self.assertTrue(
                len(config["border_color"]) > 0, f"Border Color for '{agent_id}' is empty"
            )
            self.assertTrue(
                len(config["bubble_style"]) > 0, f"Bubble style for '{agent_id}' is empty"
            )
            self.assertTrue(len(config["tone"]) > 0, f"Tone for '{agent_id}' is empty")
            self.assertTrue(
                len(config["quick_actions"]) > 0, f"Quick actions for '{agent_id}' are empty"
            )
            self.assertTrue(
                len(config["default_greeting"]) > 0, f"Greeting for '{agent_id}' is empty"
            )
    ...
    def test_cross_agent_handoff_heuristics(self):
        ...
        handoff = self.orchestrator.detect_handoff_suggestion(
            "I keep getting distracted by my phone scrolling", current_agent_id="productivity-coach"
        )
        self.assertIsNotNone(handoff)
    ...
    def test_agent_system_prompt_retrieval(self):
        ...
        for agent_id, expected_name in agents:
            ...
            self.assertIn(
                expected_name,
                prompt,
                f"System prompt for '{agent_id}' does not mention expected name '{expected_name}'",
            )
